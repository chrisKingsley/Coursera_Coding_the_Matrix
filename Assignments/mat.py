...
def matrix_matrix_mul(A, B):
    ...
    assert A.D[1] == B.D[0]
    retMat = Mat( (A.D[0],B.D[1]), { (d1,d2):0 for d1 in A.D[0] for d2 in B.D[1] } )
    
    for i in A.D[0]:
        for j in A.D[1]:
            for k in B.D[1]:
                retMat[ (i,k) ] += A[ (i,j) ]*B[ (j,k) ]
    return retMat
