...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    retVec = Vec(M.D[0], { d:0 for d in M.D[0] })
    
    for i,colVec in mat2coldict(M).items():
        retVec += v[i] * colVec
    return retVec

# M=Mat(({'a','b'}, {'@', '#', '?'}), {('a','@'):2, ('a','#'):1,('a','?'):3, ('b','@'):20, ('b','#'):10, ('b','?'):30})
# V = Vec({'@', '#', '?'}, {'@':0.5, '#':5, '?':-1})
...
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    retVec = Vec(M.D[1], { d:0 for d in M.D[1] })
    
    for i, rowVec in mat2rowdict(M).items():
        retVec += v[i] * rowVec
    return retVec
...
def Mv_mat_mat_mult(A, B):
    ...
    for colLabel,colVect in mat2coldict(B).items():
        colDict[ colLabel ] = A*colVect
    return coldict2mat(colDict)
    
# A=Mat(({'a','b'}, {'@', '#', '?'}), {('a','@'):2, ('a','#'):1,('a','?'):3, ('b','@'):20, ('b','#'):10, ('b','?'):30})
# B=Mat(({'@', '#', '?'}, {'a','b'}), {('@','a'):2, ('#','a'):1,('?','a'):3, ('@','b'):20, ('#','b'):10, ('?','b'):30})
...
def vM_mat_mat_mult(A, B):
    ...
    for rowLabel,rowVect in mat2rowdict(A).items():
        rowDict[ rowLabel ] = rowVect*B
...
def dot_prod_mat_mat_mult(A, B):
    ...
    for rowLabel,rowVect in mat2rowdict(A).items():
        for colLabel,colVect in mat2coldict(B).items():
            codomain[ (rowLabel,colLabel) ] = rowVect*colVect
    return(Mat( (A.D[0],B.D[1]), codomain))
...
# print(dot_prod_mat_mat_mult(solving_systems_m,solving_systems_a))
# print(dot_prod_mat_mat_mult(solving_systems_a,solving_systems_m))
