...
linear_dep_R_2 = [2,1/pi,sqrt(2)/2]
...
indep_vec_1 = Vec({0, 1, 2, 3},{0:1, 1:1, 2:0, 3:0})
indep_vec_2 = Vec({0, 1, 2, 3},{0:0, 1:1, 2:1, 3:0})
indep_vec_3 = Vec({0, 1, 2, 3},{0:0, 1:0, 2:1, 3:1})
indep_vec_4 = Vec({0, 1, 2, 3},{0:1, 1:0, 2:0, 3:1})
...
def is_superfluous(L, i):
    ...
    if len(L)==1:
        return False
    newMat = coldict2mat(L[:i] + L[i+1:])
    u = solve(newMat, L[i])
    residual = L[i] - newMat*u
...
# a3 = Vec({'a','b','c','d'}, {'a':1,'c':3})
# L = [a0, a1, a2, a3]
# print("is_superfluous(L, 3): %s" % is_superfluous(L, 3))
# print("is_superfluous([a0,a1,a2,a3], 3): %s" % is_superfluous([a0,a1,a2,a3], 3))
# print("is_superfluous([a0,a1,a2,a3], 0): %s" % is_superfluous([a0,a1,a2,a3], 0))
# print("is_superfluous([a0,a1,a2,a3], 1): %s" % is_superfluous([a0,a1,a2,a3], 1))
...
# vlist = [Vec({0, 1, 2},{0: 1, 1: 0, 2: 0}), Vec({0, 1, 2},{0: 0, 1: 1, 2: 0}), Vec({0, 1, 2},{0: 0, 1: 0, 2: 1}), Vec({0, 1, 2},{0: 1, 1: 1, 2: 1}), Vec({0, 1, 2},{0: 0, 1: 1, 2: 1}), Vec({0, 1, 2},{0: 1, 1: 1, 2: 0})]
# print("is_independent(vlist): %s" % is_independent(vlist))
# print("is_independent(vlist[:3]): %s" % is_independent(vlist[:3]))
# print("is_independent(vlist[:2]): %s" % is_independent(vlist[:2]))
# print("is_independent(vlist[1:4]): %s" % is_independent(vlist[1:4]))
# print("is_independent(vlist[2:5]): %s" % is_independent(vlist[2:5]))
# print("is_independent(vlist[2:6]): %s" % is_independent(vlist[2:6]))
# print("is_independent(vlist[1:3]): %s" % is_independent(vlist[1:3]))
# print("is_independent(vlist[5:]): %s" % is_independent(vlist[5:]))
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    for k,v in vec2rep(S, z).f.items():
        if v!=0 and S[k] not in A:
            return S[k]
